class TicketWorker
  ...
  def perform(*args)
    ...
    statuses.each do |status|
    	# Pull ticket count
    	tickets_count = zd_client.search(:query => "type:ticket status:#{status} updated>35minutes").count
    	# Pull all tickets
      page = 1
    	while tickets_count > 0
    	  tickets = zd_client.search(:query => "type:ticket status:#{status} updated>35minutes", :page => page).fetch!
